...
#[cfg(test)]
pub(crate) use chaos_chassis::reducer::format_error;
...
mod tests {
    ...
    fn full_turn_roundtrip() {
        ...
        assert_eq!(app.frontend.status, Status::Ready);
        assert!(app.can_submit());

        // Type + submit.
        app.update(Message::ComposerChanged("hello".to_string()));
        app.update(Message::ComposerSubmit);
        assert_eq!(app.frontend.turn, TurnState::InFlight);
        assert_eq!(app.frontend.transcript.len(), 1);
        match &app.frontend.transcript[0] {
        ...
        assert_eq!(app.frontend.transcript.len(), 2);
        assert!(matches!(
            &app.frontend.transcript[1],
            ChatEntry::Agent { .. }
        ));
        ...
        assert_eq!(app.frontend.turn, TurnState::Idle);
        assert!(app.can_submit());
    }
    ...
    fn interrupt_error_and_disconnect_paths() {
        ...
        assert_eq!(app.frontend.status, Status::Shutdown);
        assert_eq!(app.frontend.turn, TurnState::Idle);
        ...
        let len_before = app.frontend.transcript.len();
        app.update(Message::ComposerChanged("late".to_string()));
        app.update(Message::ComposerSubmit);
        assert_eq!(app.frontend.transcript.len(), len_before);
    }
    ...
    fn rich_event_rendering() {
        ...
        assert_eq!(app.frontend.transcript.len(), 1);
        assert!(matches!(
            &app.frontend.transcript[0],
            ChatEntry::Agent { .. }
        ));
        assert_eq!(app.pending_stream_count(), 1);
        ...
        assert_eq!(app.pending_stream_count(), 0, "finalize clears pending key");
        ...
        assert_eq!(
            app.frontend.transcript.len(),
            len_before,
            "TokenCount must not log"
        );
        assert!(app.frontend.token_usage.is_some());
    ...
    fn finalize_reconciliation_and_call_leaks() {
        ...
        assert_eq!(app.pending_stream_count(), 0);
        assert_eq!(app.pending_call_count(), 0);
        ...
        let idx_before = app.frontend.transcript.len();
        ...
        assert!(app.frontend.transcript.len() > idx_before + 1);
        match app.frontend.transcript.last() {
            Some(ChatEntry::Exec {
                exit_code: None,
                command,
        ...
        assert_eq!(app2.frontend.turn, TurnState::InFlight);
        ...
        match app2.frontend.transcript.last() {
            Some(ChatEntry::Notice {
    ...
    fn terminal_error_cleans_bookkeeping_and_failed_submit_keeps_draft() {
        ...
        let transcript_len = app2.frontend.transcript.len();
        app2.update(Message::ComposerSubmit);
        ...
        assert_eq!(
            app2.frontend.transcript.len(),
            transcript_len + 1,
            "kernel death should log one shutdown notice and no User entry"
        );
        assert!(matches!(
            app2.frontend.transcript.last(),
            Some(ChatEntry::Notice {
                level: NoticeLevel::Error,
                ..
            })
        ));
        assert_eq!(app2.frontend.status, Status::Shutdown);
        assert_eq!(app2.frontend.turn, TurnState::Idle);
    }
    ...
    fn theme_palette_and_view_smoke() {
        ...
        let phosphor =
            crate::theme::palette_for_family(chaos_chassis::theme::ThemeFamily::Phosphor);
        let anthropic =
            crate::theme::palette_for_family(chaos_chassis::theme::ThemeFamily::Anthropic);
        ...
        assert!(anthropic.fg.r > anthropic.fg.b);
        ...
        assert!(
            app.frontend.transcript.iter().any(|e| matches!(
                e,
                ChatEntry::Tool {
                    result: Some(Ok(_)),
                    ..
