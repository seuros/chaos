...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: chaos_ipc::protocol::SessionConfiguredEvent) {
        self.bottom_pane
    ...
    fn on_guardian_assessment(&mut self, ev: GuardianAssessmentEvent) {
        ...
        let cell = if let Some(command) = guardian_command(&action) {
            history_cell::new_approval_decision_cell(
                command,
                chaos_ipc::protocol::ReviewDecision::Denied,
                history_cell::ApprovalDecisionActor::Guardian,
            )
        } else {
            match tool {
    ...
    fn on_patch_apply_end(&mut self, event: chaos_ipc::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
    ...
    fn on_hook_started(&mut self, event: chaos_ipc::protocol::HookStartedEvent) {
    ...
    fn on_hook_completed(&mut self, event: chaos_ipc::protocol::HookCompletedEvent) {
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: chaos_ipc::protocol::PatchApplyEndEvent,
    ) {
    ...
    fn pending_steer_compare_key_from_item(
...
fn hook_event_label(event_name: chaos_ipc::protocol::HookEventName) -> &'static str {
    match event_name {
