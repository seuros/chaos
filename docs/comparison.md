...
| **MCP support** | Client + server, `.mcp.json` | Partial (no elicitation, limited resources) | Client + server, tools, resources, OAuth |
...
| **Phone home** | No update checker; optional OTLP export | Telemetry (opt-out) | Update checker + telemetry |
...
It ships as a small set of native binaries with no update checker or mandatory
outbound telemetry. Local tracing is built in; OpenTelemetry/OTLP export is
available when configured. It runs on Linux, macOS, and FreeBSD.
