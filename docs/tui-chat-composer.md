...
(`tui/src/bottom_pane/chat_composer.rs`). The default config preserves current chat
behavior.
...
`tui/src/bottom_pane/slash_commands.rs` and reused by both the composer and the command
