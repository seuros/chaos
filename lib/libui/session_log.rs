...
pub fn maybe_init(config: &Config) {
    let enabled = std::env::var("CHAOS_TUI_RECORD_SESSION")
    ...
    let path = if let Ok(path) = std::env::var("CHAOS_TUI_SESSION_LOG_PATH") {
        PathBuf::from(path)
    } else {
        let mut p = match chaos_kern::config::log_dir(config) {
            Ok(dir) => dir,
