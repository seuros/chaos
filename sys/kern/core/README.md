...
Expects the binary containing `codex-core` to run the equivalent of `codex sandbox linux` (legacy alias: `codex debug landlock`) when `arg0` is `alcatraz-linux`. See the `codex-arg0` crate for details.
