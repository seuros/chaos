...
fn map_api_error_maps_server_overloaded_from_503_body() {
    ...
    let err = map_api_error(ApiError::Transport(TransportError::Http {
        status: rama::http::StatusCode::SERVICE_UNAVAILABLE,
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        rama::http::HeaderValue::from_static("chaos_other"),
    );
    headers.insert(
    ...
    .to_string();
    let err = map_api_error(ApiError::Transport(TransportError::Http {
        status: rama::http::StatusCode::TOO_MANY_REQUESTS,
        url: Some("http://example.com/v1/responses".to_string()),
...
fn map_api_error_extracts_identity_auth_details_from_headers() {
    ...
    headers.insert(
        REQUEST_ID_HEADER,
        rama::http::HeaderValue::from_static("req-401"),
    );
    headers.insert(
        CF_RAY_HEADER,
        rama::http::HeaderValue::from_static("ray-401"),
    );
    headers.insert(
        X_OPENAI_AUTHORIZATION_ERROR_HEADER,
        rama::http::HeaderValue::from_static("missing_authorization_header"),
    );
    let x_error_json =
        base64::engine::general_purpose::STANDARD.encode(r#"{"error":{"code":"token_expired"}}"#);
    headers.insert(
        X_ERROR_JSON_HEADER,
        rama::http::HeaderValue::from_str(&x_error_json).expect("valid x-error-json header"),
    );
