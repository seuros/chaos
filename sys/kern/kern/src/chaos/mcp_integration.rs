...
/// Whether a failed MCP operation should count against the circuit breaker.
///
/// Deterministic server/application errors (`GuestError::Server`, invalid
/// params, protocol violations, …) are not signs of an unhealthy server and
/// must not trip the breaker. Only transient transport/timing failures signal
/// a server that may be unavailable; those are the only failures the breaker
/// exists to shed load against.
///
/// `GuestError::is_retryable()` already encodes this distinction. The error is
/// reachable here because `chaos_mcp_runtime` re-exports `GuestError` and the
/// manager preserves the error type with `anyhow::Error::from` instead of
/// stringifying it.
fn is_transient(error: &anyhow::Error) -> bool {
    error
        .downcast_ref::<chaos_mcp_runtime::GuestError>()
        .is_some_and(chaos_mcp_runtime::GuestError::is_retryable)
}
    ...
    Fut: Future<Output = anyhow::Result<T>>,
{
        ...
        if guard.breaker.is_open() {
            if let Some(opened_at) = guard.opened_at {
                if opened_at.elapsed() >= HALF_OPEN_TIMEOUT {
                    ...
                    // The manual record_* API has no Open->HalfOpen transition,
                    // so this goes straight to Closed; see struct docs.
                    guard.breaker.reset();
...

    {
        ...
        match &result {
            ...
            // Deterministic application/server errors do not signal an
            // unhealthy server; leave breaker state untouched.
            Err(_) => {}
        }
    }
