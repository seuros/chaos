        ...
        if let Some(files) = maybe_files {
            ...
            let patch = changes
                ...
                .map(|(path, change)| match change {
                    chaos_ipc::protocol::FileChange::Add { content } => {
                        format!("*** Add File: {}\n{}", path.display(), content)
                    }
                    chaos_ipc::protocol::FileChange::Delete { content } => {
                        format!("*** Delete File: {}\n{}", path.display(), content)
                    }
                    chaos_ipc::protocol::FileChange::Update {
                        unified_diff,
                        move_path,
                    } => {
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            chaos_ipc::request_user_input::RequestUserInputAnswer {
