...
fn read_only_vfs_policy() -> VfsPolicy {
...
fn unrestricted_vfs_policy() -> VfsPolicy {
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    requirements_exec_policy.add_network_rule(
        "blocked.example.com",
        chaos_selinux::NetworkRuleProtocol::Https,
        Decision::Forbidden,
        None,
    )?;

    let requirements = ConfigRequirements {
        exec_policy: Some(chaos_sysctl::Sourced::new(
            chaos_sysctl::RequirementsExecPolicy::new(requirements_exec_policy),
            chaos_sysctl::RequirementSource::Unknown,
        )),
        ..ConfigRequirements::default()
    };
...
fn unmatched_on_request_uses_split_filesystem_policy_for_escalation_prompts() {
    let command = vec!["madeup-cmd".to_string()];
    let restricted_file_system_policy = VfsPolicy::restricted(vec![]);
