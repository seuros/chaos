...
pub(crate) fn is_guardian_reviewer_source(
    ...
    matches!(
        session_source,
        chaos_ipc::protocol::SessionSource::SubAgent(SubAgentSource::Other(name))
...
pub(super) async fn run_guardian_review_session(
    session: Arc<Session>,
    turn: Arc<TurnContext>,
    prompt_items: Vec<chaos_ipc::user_input::UserInput>,
    ...
    let preferred_reasoning_effort = |supports_low: bool, fallback| {
        if supports_low {
            Some(chaos_ipc::openai_models::ReasoningEffort::Low)
        } else {
            fallback
        }
    };
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = preferred_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset
                ...
                .iter()
                .any(|effort| effort.effort == chaos_ipc::openai_models::ReasoningEffort::Low),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| preset.effort == chaos_ipc::openai_models::ReasoningEffort::Low),
