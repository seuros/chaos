...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            chaos_proc::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
            ...
            chaos_proc::Phase2JobClaimOutcome::SkippedNotDirty => return Err("skipped_not_dirty"),
            chaos_proc::Phase2JobClaimOutcome::SkippedRunning => return Err("skipped_running"),
        };
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[chaos_proc::Stage1Output],
        reason: &'static str,
    ) {
        session.services.session_telemetry.counter(
            metrics::MEMORY_PHASE_TWO_JOBS,
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[chaos_proc::Stage1Output],
) -> i64 {
    latest_memories
        .iter()
