...
use chaos_vault::redact_secrets;
...
async fn claim_startup_jobs(
...
) -> Option<Vec<chaos_proc::Stage1JobClaim>> {
    ...
    match state_db
        .claim_stage1_jobs_for_startup(
            ...
            chaos_proc::Stage1StartupClaimParams {
...
async fn run_jobs(
    session: &Arc<Session>,
    claimed_candidates: Vec<chaos_proc::Stage1JobClaim>,
...
mod job {
    ...
    pub(in crate::memories) async fn run(
        session: &Session,
        claim: chaos_proc::Stage1JobClaim,
        stage_one_context: &RequestContext,
    ) -> JobResult {
