...
impl fmt::Display for SkillParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            SkillParseError::Read(error) => write!(f, "failed to read file: {error}"),
            SkillParseError::MissingFrontmatter => {
                write!(f, "missing YAML frontmatter delimited by ---")
            }
            SkillParseError::InvalidYaml(error) => write!(f, "invalid YAML: {error}"),
...
fn skill_roots_from_layer_stack_inner(
    ...
    for layer in config_layer_stack.get_layers(
        ...
        match &layer.name {
            ConfigLayerSource::Project { .. } => roots.push(SkillRoot {
            ...
            ConfigLayerSource::System { .. } => roots.push(SkillRoot {
                path: config_folder.as_path().join(SKILLS_DIR_NAME),
                scope: SkillScope::Admin,
            }),
            ConfigLayerSource::ProjectMcp { .. } | ConfigLayerSource::SessionFlags => {}
        }
    }
...
fn discover_skills_under_root(root: &Path, scope: SkillScope, outcome: &mut SkillLoadOutcome) {
    ...
    while let Some((dir, depth)) = queue.pop_front() {
        let entries = match fs::read_dir(&dir) {
            Ok(entries) => entries,
            Err(error) => {
                error!("failed to read skills dir {}: {error:#}", dir.display());
                continue;
            }
        };
        ...
        for entry in entries.flatten() {
            let path = entry.path();
            let file_name = match path.file_name().and_then(|file| file.to_str()) {
                Some(name) => name,
                None => continue,
            };
            ...
            if file_type.is_symlink() {
                ...
                let metadata = match fs::metadata(&path) {
                    ...
                    Err(error) => {
                        error!(
                            "failed to stat skills entry {} (symlink): {error:#}",
                            path.display()
                        );
                        continue;
                    }
...
fn resolve_color_str(value: Option<String>, field: &'static str) -> Option<String> {
    ...
    if value.len() == 7 && chars.next() == Some('#') && chars.all(|char| char.is_ascii_hexdigit()) {
