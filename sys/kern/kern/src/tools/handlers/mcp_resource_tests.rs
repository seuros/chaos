...
fn inline_resources_include_builtin_resources() {
    ...
    let machine = resources
        ...
        .expect("machine resource is discoverable");
    assert_eq!(machine.name, "machine");
    assert_eq!(
        machine.mime_type.as_deref(),
    ...
    assert_eq!(resources[3].uri, builtin_mcp_resources::CHAOS_MODELS_URI);
...
async fn machine_resource_reads_are_fresh_and_use_the_active_turn_directory() {
    ...
    config.cwd = root.path().join("unused-startup-cwd");
    ...
    for name in ["first-workspace", "second-workspace"] {
        turn.cwd = root.path().join(name);
        std::fs::create_dir_all(&turn.cwd).expect("create workspace");
        let result = read_resource_contents(
            &session,
            &turn,
            None,
            builtin_mcp_resources::CHAOS_MACHINE_URI,
        )
        .await
        .expect("read machine resource");
        let ResourceContents::Text(content) = &result.contents[0] else {
            panic!("expected machine JSON text");
        };
        assert_eq!(content.uri, builtin_mcp_resources::CHAOS_MACHINE_URI);
        ...
        let observed_at = value["machine"]["observed_at"].clone();
        assert!(observed_at.is_object());
        assert_ne!(previous_observed_at.as_ref(), Some(&observed_at));
        previous_observed_at = Some(observed_at);
        let workspace_paths: Vec<_> = value["storage"]["filesystems"]
            .as_array()
            .expect("filesystem observations")
            .iter()
            .flat_map(|filesystem| filesystem["targets"].as_array().expect("targets"))
            .filter(|entry| entry["target"]["role"] == "workspace")
            .map(|entry| entry["target"]["path"].clone())
            .collect();
        assert_eq!(workspace_paths, vec![json!(turn.cwd)]);
        assert!(value["storage"]["unavailable"].is_array());
    }
}
