...
fn allowlist_matches_exact_hosts_and_parses_connect_targets() {
    ...
    assert!(!policy.permits("www.googleapis.com.evil.test"));
    assert!(policy.permits("lh3.googleusercontent.com"));
    assert!(!policy.permits("lh3.googleusercontent.com.evil.test"));
    assert!(!policy.permits("lh4.googleusercontent.com"));
