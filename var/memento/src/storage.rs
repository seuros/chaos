...
async fn rebuild_raw_memories_file(
    ...
    for memory in retained {
        ...
        writeln!(body, "process_ref: {}", memory.process_ref).map_err(raw_memories_format_error)?;
...
async fn write_rollout_summary_for_thread(
    ...
    writeln!(body, "process_ref: {}", memory.process_ref).map_err(rollout_summary_format_error)?;
